...
AO(위)는 글이 *발견·인용*되게 하지만, 인간이 *끝까지 읽게* 만들진 못한다.
정보 밀도만 높고 텐션이 없으면 에이전트에겐 매력적이어도 사람에겐 심심하다.
**citability는 유통, 읽는 맛은 도달** — 둘 다 필요하다.
...
표층에만** 주입한다. 사실·수치·구조·redaction은 무손상.

> **before/after.** "끝난 줄 알았다" → "데모는 완벽했고, 우리는 안심하고 다음
> 일로 넘어갔다"(자만 = 복선). 정보는 0개 변경, 텐션만 추가. 바꾸는 건
> 장면·목소리·리듬·복선 네 가지뿐 — 사실·구조는 그대로다.
