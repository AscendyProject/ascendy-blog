...
- 그 뒤 **백엔드가 prod 구조화 로그 + DB 집계를 hand-off.** 세 가지가 한 번에 결판:
  1. **새 사진이 서버에 *도달조차* 안 함** — 클라이언트가 보낸 목록에 새 항목이 하나도 없음. 백엔드는 깨끗.
  2. **라이브러리가 client scan cap을 넘김** — 클라이언트가 *고정 크기 윈도우*(상위 일부)만 스캔하는데,
     라이브러리가 그 cap을 넘으면 그 윈도우가 라이브러리 전체를 더는 못 덮는다 → 윈도우 밖 사진은
     스캔에 *아예 안 보임* → 서버에 보낼 목록에 못 들어감. (prod 증거: 라이브러리가 cap을 넘었고, 새 항목이 서버에 도달하지 않음.)
...
- **리뷰어(독립 LLM)가 v1에서 결함 D1을 잡음 — cursor가 tie-safe가 아니다.** cursor 키를
  *타임스탬프 하나*로만 쓰면, **같은 시각에 청크 크기 이상의 사진이 몰리는 경우**(외부 메신저 앱
  ...
  - 트레이스: 같은 타임스탬프 사진이 청크 크기보다 많을 때, native가 매번 *어떤* 청크를 주는지는
    ...
    plateau 나머지 영구 누락.
  - **lint/typecheck/*기존* 단위테스트가 못 잡은 결함**(그 데이터 분포를 모델링한 테스트 없이는 안 보임) — 코드 본문엔 안 보이고, *데이터 분포를 상상*해야
