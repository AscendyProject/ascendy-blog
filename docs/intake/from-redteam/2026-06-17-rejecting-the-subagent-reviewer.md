...
자기-리뷰 붕괴 가드는 한쪽(worker)을 *정규화된 provider family*로, 다른 쪽(reviewer)을 *raw 어댑터
식별자*로 비교했다. 형식이 어긋난 비교라, 새 어댑터를 들이면 그 동일성 검사가 **cross-provider인 척
통과 → 같은 프로바이더의 자기-리뷰가 가드를 우회**할 수 있었다. plan_review에서 HIGH로 잡혔고,
결정문에 *하드 선결조건*으로 박았다: 비교 전에 **양쪽을 같은 정규형(family)으로 맞춘다.**
(literal 키·비교 코드는 본문에 싣지 않음 — exploit 단계 미서술 기준.)
→ 교훈: **한쪽은 정규화된 family로, 다른 쪽은 raw 식별자로 비교하면, 형식 차이를 파고드는 입력 하나로
동등성 검사가 뚫린다.** 비교 전에 양쪽을 같은 정규형으로. (식별자·권한·동일성 검사 일반에 적용.)
