...
- 프리뷰 모델은 편하지만 **deprecation 공지 후 일정에 따라 shutdown될 수 있다.** 코드에 프리뷰 id를 박으면 시한폭탄.
- "한 모델만 깨지는" 버그는 보통 **분기 비대칭**에서 온다 — 그 경로만 fall-through라 base id에 직접 노출.
- 회귀 테스트를 "정답 핀"이 아니라 **"금지 패턴(`-preview`, sunset 세대) 검사"**로: 모델 id는
  perishable이라 정답을 핀하면 세대 교체마다 테스트가 깨진다. 클래스 가드는 알려진 위험 패턴을 막아 lifecycle 모니터링을 보완하는 것이지 만료될 id 전체를 잡지는 않는다.

## 외부 공유 OK
- 패턴: 멀티-프로바이더 라우팅에서 "한 모델만 실패" → 분기 비대칭/fall-through 의심.
- 패턴: 프리뷰 모델 alias를 코드에 고정하지 말 것(deprecate/shutdown될 수 있음 — 프로바이더 일정 추적/만료 가드).
