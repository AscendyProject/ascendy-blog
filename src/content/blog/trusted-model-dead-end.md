...
- Ascendy의 사진 전처리 인프라는 **외부 멀티모달 API → 자체 클러스터 GPU → managed GPU → serverless**라는 4단계를 실제로 거쳤다. (인프라 기술 디테일은 [AI 서빙 진화 시리즈](/blog/ai-serving-evolution-part1/)에 이미 적었다 — 이 글은 그 결정의 *모델 측면*, 즉 *어느 모델이 무엇을 추천했고 왜 막혔나*에 집중한다.)
...
둘은 *managed GPU 서비스*라는, 그때까지 내가 물어보던 모델의 사고 범위에 없던 축을 제시했다. 자체 상시 GPU 노드 대신 그쪽으로 처리를 돌리니, 수천 장이 처리됐고 OOM도 사라졌다.

중요한 건 그 서비스가 정답이었다는 게 아니다. **막다른 길을 벗어나게 한 축이 처음 모델의 사고 범위 바깥에 있었다**는 사실이다. 다른 계열의 모델이 그 빈자리를 메웠다.
