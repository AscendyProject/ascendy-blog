...
## 안 하기로 한 것 — `/rating`은 "상위 X%"를 설계상 거부한다

`/rating`에서 가장 하고 싶은 말은 이게 **하지 않기로 한 것**이다.

`/rating`은 "당신은 전체 개발자 중 **상위 N%**입니다" 같은 모집단 비교를 *설계상* 만들지 않는다. 그렇게 말하려면 *남들과 비교한 데이터*가 있어야 하는데, 그게 없다. **없는 걸 말하면 그게 곧 지어내기다.** 그래서 등급은 어디까지나 *내 증거 자체*에 대한 평가지 남과의 순위가 아니다 — 비교할 모집단 데이터가 없으니 *정당한* percentile은 계산되지조차 않고, 프롬프트도 그런 표현을 금지하며, 결정론 렌더러 자신의 출력(템플릿 + "모집단 내 위치가 아니다"라는 면책 문구)엔 percentile·rank 어휘가 없다. 회귀 테스트(`test_no_percentile_lexicon_in_rendered_output`)가 기본 렌더 출력에 그 어휘가 없음을 지킨다.

다만 정직하게 한계를 적자면 — **모델이 쓴 reasoning 텍스트는 사후 검증하지 않는다.** 렌더러는 그 텍스트를 그대로 출력하므로, 모델이 근거 없이 "상위 1%"를 써넣으면 그건 스코어카드에 남을 수 있다. 즉 모집단 데이터 부재는 *정당한* percentile 계산을 불가능하게 할 뿐, 모델 출력까지 원천 차단하는 하드 개런티는 아니다. "안 한다"는 건 *설계·프롬프트·결정론 경로의 의도*지, 출력 수준에서 봉인된 보장이 아니다.
