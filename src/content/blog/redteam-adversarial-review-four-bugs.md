...
그리고 출하와 비-출하를 섞지 않기 위해 — 0.3에 들어간 건 리뷰어 **fallback ladder 단계**다(issue #37 step 4). 같은 #37의 나머지 두 갈래(sub-agent reviewer adapter, 터미널 멀티플렉서 스크린스크래핑 트랜스포트)는 그 뒤 **설계상 거부**됐다(결정문 PR #68 머지, #37·#67 모두 CLOSED). 즉 "아직 로드맵"이 아니라 **"안 만들기로 결정"**이다 — 그 거부 결정 자체를 다룬 글이 [기능을 거부하는 법](/blog/how-to-reject-a-feature/)이다.
