---
title: "A component vanished quietly — the cost of half-following a convention"
description: "An invite QR rendered as a blank white box. The culprit was the component auto-import naming rule — in a repo that mostly used explicit imports and had therefore never verified how it works."
pubDate: 2026-08-15
...
- An invite QR code rendered as a **blank white box.** Not an error, not an empty page — a box that took up its space with nothing inside it. **Dev looked identical**; the only difference was a single console warning (which nobody was reading).
...
- So the build couldn't resolve that tag to a static import and **left it as a runtime lookup**, and an unresolvable name renders as an unresolved custom element — **silently.**
- What redirected us was **the bundle.** Grepping the build output for **a string unique to the QR library** turned up nothing. That isn't proof on its own, but it shook the "the library failed" story and sent us to **the compiled output and the registry.**
...
- **When a problem splits along environments, read the build output.** A library's unique strings are a good starting point — but **a lead, not a verdict.** A missing string has plenty of other explanations: minification, server-only chunks, externalization. Settle it with the compiled output and the registry.
- **Distinguish "it failed" from "it never ran at all."** That distinction is what turned the hypothesis around.
