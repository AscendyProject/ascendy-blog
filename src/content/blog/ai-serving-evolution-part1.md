...
그리고 여기에 **고정비용**이 얹혔다. 자체 클러스터의 GPU 노드는 always-on이다. 트래픽이 없어도 GPU 2장이 시간당 과금된다. multi-model 서빙의 운영 부담 — OOM 튜닝, 모델 로딩, 노드 장애 대응 — 에 always-on 고정비용까지 더해지자, "자체 서빙이 클라우드 API보다 싸다"는 처음의 가정이 흔들렸다.
