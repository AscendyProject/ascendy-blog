...
description: "The easy trap in a Nuxt SEO baseline: og:url and canonical have no meaningful default. With one global default, every page advertises root and search drops them as duplicates. Per-page override is mandatory not optional."
pubDate: 2026-06-12
...
- The point: **`og:url` and `<link rel="canonical">` are fields with *no meaningful default*.** They must differ per page; ship only a global default and (1) every page's social preview advertises the same root, and (2) search engines treat every page as a *duplicate* of root and drop them from the index.
...
2. **Search indexing** — the more lethal one. When `<link rel="canonical">` points to root on every page, **search engines judge about/pricing/privacy/terms/licenses to be *duplicates* of root.** Canonical is a declaration of "this is the original"; if everyone points to root as the original, *the rest of the pages fall out of the index.* You set up an SEO baseline and quietly delete your pages from search.
...
18 lines across 6 pages. That's all it was — a small omission, but with an outsized result ("every page disappears from search").
...
- **`og:url` and `canonical` are fields with no meaningful default.** They must differ per page, so a global default alone is *always* wrong for them. Per-page override isn't optional — it's mandatory.
- **A global default OG is a safe start — except these two fields.** Apply the rest-of-OG convention to them and every page advertises root and drops out of search as a duplicate.
- **If canonical points to root everywhere, those pages vanish from the index.** An own-goal: setting up SEO while erasing SEO.
